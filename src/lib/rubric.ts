...
- priority_explanation and every next_steps[].observation (weak/mid bands, score under 7.5, only; the 7.5-10.0 strong band above stays praise-only, 2-3 sentences, per that band's own rules) follow the SAME two-part structure, 3-4 short sentences TOTAL, never an essay:
  ...
  PROP RULE (click_appeal/presentation advice only): a suggested prop must be exactly ONE small item that is actually related to how the product is used (a washcloth near soap, a spoon near a candle in a jar, a bookmark near a journal) — never generic decoration (flowers, ribbons, random items) with no connection to the product. State it is ONE item, kept to the side, so the product stays the clear focus. This matters because the SAME rubric penalizes clutter and distracting objects elsewhere: a prop suggestion that would itself read as clutter on the next photo is a contradiction and must never be given.
