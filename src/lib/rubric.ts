...
- Margin and square-crop safety for main photos: if the photo appears intended to show the whole product, drop Thumbnail below 8 when it is zoomed so tightly that there is no breathing room or an Etsy square crop would clip a key edge. If a cup, saucer, clasp, edge, bundle piece, or other product context touches/runs off the image border in a full-product hero view, do not call the photo strong even when the product is recognizable. A strong main thumbnail shows the intended product view with enough margin to survive Etsy's square crop.
- Wording: truncation and tight margin are DIFFERENT findings and must never share language. Use "cut off" / "partially cut off" / "pull back and re-shoot" ONLY when part of the product is actually missing from the image. When the product is fully visible but an edge or piece merely touches or sits too close to the border, say that truthfully instead — e.g. "The clasp touches the top edge. Leave more breathing room so Etsy's square crop does not clip it." A seller who can SEE their complete product must never be told it is cut off.
- Intentional macro/detail shots are different: a tight locket face, engraving, gemstone, clasp, label, texture, or small design detail is not "cropped wrong" just because it is close. Grade it as the uploaded image: it may be a strong detail/supporting photo, but as a main hero thumbnail it can lose points if buyers cannot understand the full item being sold. In that case, say it works as a detail photo and recommend adding or using a separate full-product hero, not "zoom out" as if the macro itself is broken.
- For cups, mugs, teacup candles, and saucer setups: when the photo is intended as the full hero view, the full cup body, handle, rim, and saucer if present should be visible with margin. A clipped handle or saucer in a full-product hero view cannot score Thumbnail 8+.
- NECKLACE EXCEPTION (necklaces and neck chains ONLY — no other product type): the clasp or the back of the chain cropped out of frame is NORMAL, expected necklace framing. It is at most a minor note, never the priority finding and never a hard Thumbnail penalty. Punish a necklace's framing only when the cropping hides the design itself: a cut-off pendant or focal, a bead arrangement whose pattern cannot be understood, or missing pieces that make "what am I buying" unclear.
- Every OTHER product keeps the full incomplete-framing rules above: cut-off bracelets (their full loop and closure matter), clipped mug handles or saucers, cropped bundle pieces, and any product whose extent runs off the frame are still punished and remain the priority finding. The exception is for necklaces, not for jewelry in general.
