...
- If the photo is UNRELATED to the listing — a different PRODUCT CATEGORY entirely, a different brand, or a random object that could not be part of this listing — classify supporting_photo_role "unrelated_or_wrong_product". Set buyer_question_answered "" and supporting_verdict a plain sentence like "This photo shows a different product, not the <listing item>." Score Buyer Confidence 0-1, and keep the other pillars low; the overall must land near 0 (weak band). priority_action tells the seller to upload a photo of the actual <listing item> or its packaging/details. Different PRODUCT CATEGORY means a different type of item — a mug shown in a towel listing, a plush toy shown in a candle listing, a necklace shown in a mug listing.
- A different color, pattern, print, material finish, size, or added accessory/prop on the SAME item type is NEVER wrong-product — sellers routinely show color and style options within one listing (the same tote in a different fabric print, the same plush wearing a different hat or no hat, the same mug in a different glaze color). Classify these as supporting_photo_role "variation" instead and score them normally against their role, same as any other supporting photo.
